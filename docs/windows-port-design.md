...
| **CoreGraphics**（`CGEvent`/`CGShieldingWindowLevel`，后者已弃用→等价 `.screenSaver`） | ❌ **完全不可用**<sup>[[3]](#ref3)</sup> | macOS 专有，Windows 无对应物 |
...
### 2.3 .NET → macOS 反向跨平台的循证结论（2026-06 补）

§2.1–2.2 论证了「**Swift→Windows**」不可行；本节补全互补方向「**.NET→macOS**」的结论，回答「能否把 .NET 这套收敛为单代码库、同时产出 macOS + Win10 构建」：

- **运行时 / 纯类库层跨平台成立**：纯 `net8.0` 程序集（如 `GiveMeABreakEngine` C# FSM 核心）同一份 DLL 在 macOS/Windows/Linux 均可运行，双端 CI 已验证（`dotnet-test`(windows) + `dotnet-test-mac`(macos)）。
- **WPF 是 Windows 永久专属**：`net8.0-windows` 物理上产不出 macOS 构建（`GiveMeABreakShell.csproj:8`），微软官方立场未变；WPF/WinForms 永不跨平台。
- **「单一 .NET UI 代码库覆盖双端全部能力」不可行**：本项目 9 项 macOS 标志能力（尤其 `NSStatusItem` 菜单栏驻留 + 全屏遮罩）不在任何跨平台 UI 抽象内，必须 macOS 原生 interop（`dotnet/macios` 绑定 / P-Invoke）。唯一号称单代码库的 **MAUI 走 Mac Catalyst（UIKit）**，恰恰做不了这两项核心（`NSStatusItem` 是纯 AppKit 类，Catalyst 不暴露）；其 AppKit backend（`maui-labs`）仍为实验性、非微软官方。**Avalonia** 是 .NET 生态最可行方案（`TrayIcon` 源码级即原生 `NSStatusItem`，Windows/macOS 同 XAML），但平台集成层（遮罩 / 媒体键 / 音频 / 自启 / 辅助功能）仍需厚 interop，等价于「用 C# 重写一遍 Swift 平台层」。
- **结论（与 §2 对称）**：可单代码库共享的只有纯引擎，UI 壳 + 平台集成层注定双写。当前「原生 Swift（macOS）+ 原生 WPF（Win10）+ 共享纯引擎语义 + 黄金测试防漂移」**即该结论的最优实现**，不建议为「单代码库」迁移 macOS 侧到 .NET（迁移成本 > 仅省数百行纯引擎双份，且 macOS 原生保真度反降）。

> **若未来考虑 Avalonia 的两条注意事项**（循证校准，免踩坑）：① 隐藏 Dock 图标做纯菜单栏 app，用官方一等公民 API `new MacOSPlatformOptions { ShowInDock = false }`（底层即 `setActivationPolicy(.accessory)`），**无需**手写 P/Invoke `setActivationPolicy`（旧社区建议已过时）；② 全屏遮罩经 `TryGetPlatformHandle()` 取 `NSWindow` 句柄手设 `.screenSaver` level + `NSApplication.Activate()` 转前台——仍属手写 interop，非开箱即用。完整循证（5 维度调研 + 14 条对抗性验证）见决策 plan。
...
这是整个移植的**阿喀琉斯之踵**。macOS 的 `CGShieldingWindowLevel()`（⚠️ Apple 已弃用，现代等价 `CGWindowLevelForKey(kCGShieldingWindowLevelKey)` / `NSWindow.Level.screenSaver`，同屏蔽层级——此处的「Windows 无等价物」指 Windows 平台无对应屏蔽级 API，与 Apple 侧弃用无关）能压过 Dock、菜单栏、全屏应用、屏保<sup>[[3]](#ref3)</sup>；Windows **没有任何单一 API 等价物**，`HWND_TOPMOST` 在多场景盖不住 taskbar<sup>[[12]](#ref12)</sup>。故采用**妥协分层**，诚实接受「软强制」本质（对齐 macOS 版 Esc 二次确认的设计哲学，见 [issue #2](../.agents/issue.md)）：
...
<a id="ref3"></a>[3] Apple Inc., "CGShieldingWindowLevel()（已弃用；等价 `CGWindowLevelForKey(kCGShieldingWindowLevelKey)` = `NSWindow.Level.screenSaver`）/ CGEvent — Core Graphics Reference," *Apple Developer Documentation*, 2026. [Online]. Available: https://developer.apple.com/documentation/coregraphics/cgshieldingwindowlevel()
